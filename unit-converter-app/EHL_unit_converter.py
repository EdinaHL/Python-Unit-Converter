...
from kitchen_units_converter import Kitchen_Units as KUCon
...
# ---------------------- Category section ----------------------
category_frame = Frame(main_window, bg="seashell2", pady=10, highlightbackground="seashell3", highlightthickness=1)
...
from_frame = Frame(conversion_units, bg="seashell2", highlightbackground="seashell3", highlightthickness=1)
from_frame.pack(expand=True, fill=BOTH, side=LEFT)
to_frame = Frame(conversion_units, bg="seashell2", highlightbackground="seashell3", highlightthickness=1)
...
def display_capacity():
    ...
    convert_from_label = Label(from_frame, text="Convert from unit:", bg="seashell2")
    ...
    convert_to_label = Label(to_frame, text="Convert to unit:", bg="seashell2")
...
def display_kitchen_units():
    clear_from_to_units_frame()
    from_kitchenunits = StringVar(value="ml (millilitres)")
    KUCon.from_this_unit(new_kucon, from_kitchenunits.get())
    convert_from_label = Label(from_frame, text="Convert from unit:", bg="seashell2")
    convert_from_label.pack()
    for unit in KUCon.kitchen_units_list:
        units_rb = Radiobutton(from_frame, text=unit, variable=from_kitchenunits
                               , value=unit, bg="seashell2"
                               , command=lambda: KUCon.from_this_unit(new_kucon, from_kitchenunits.get()))
        units_rb.pack()
    to_kunits = StringVar(value="ml (millilitres)")
    KUCon.to_this_unit(new_kucon, to_kunits.get())
    convert_to_label = Label(to_frame, text="Convert to unit:", bg="seashell2")
    convert_to_label.pack()
    for unit in KUCon.kitchen_units_list:
        units_rb = Radiobutton(to_frame, text=unit, variable=to_kunits
                               , value=unit, bg="seashell2"
                               , command=lambda: KUCon.to_this_unit(new_kucon, to_kunits.get()))
        units_rb.pack()
...
def calculate_result():
    ...
    if (CapCon.user_input_value(new_capacity, input_entry.get()) == "Invalid Number"
            ...
            or KUCon.user_input_value(new_kucon, input_entry.get()) == "Invalid Number"):
    ...
    if selected_category.get() == "Kitchen Units":
        output_entry.delete(0,100)
        KUCon.calculation(new_kucon, input_entry.get())
        result = KUCon.return_result(new_kucon)
        output_entry.insert(0, result)
